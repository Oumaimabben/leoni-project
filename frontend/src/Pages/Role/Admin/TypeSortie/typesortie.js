...
const Typesortie = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (fila) => (
        <>
          <Button  type="primary" style={{ background: "green",border:"0px" }} onClick={() => showModal(fila.id)}>
            modifier
          </Button>
        </>
      ),
  ...
  const handleConfirmReservation = (values) => {
    axios
      .patch(
        "http://localhost:8800/typesortie/" + seletectedItem,
        { ...values },
        {
          headers: {
      ...
      .then((data) => {
        notification.success({ message: "type de sortie modifiée avec succés" });
        form.resetFields();
        handleCancel();
        getData();
      })
      .catch((err) => {
        notification.open({
          message: "vérifier vos information le type de sortie doit étre unique",
          type: "error",
        });
        form.resetFields();
        handleCancel();
  ...
  const getData = () => {
    axios
      .get("http://localhost:8800/typesortie", {
        headers: {
          authorization: bearer,
        },
      })
  ...
  const deleteData = (id) => {
    axios
      .delete(`http://localhost:8800/typesortie/${id}`, {
        headers: {
          authorization: bearer,
        },
      })
