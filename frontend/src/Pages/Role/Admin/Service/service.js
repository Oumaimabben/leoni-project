...
import { Button, Form, Input, Modal, Table, Space, notification,InputNumber } from "antd";
...
const Services = () => {
  ...
  const handleConfirmReservation = (values) => {
    axios
      .patch(
        "http://localhost:8800/service/" + seletectedItem,
        { ...values },
        {
          headers: {
  ...
  const deleteData = (id) => {
    axios
      .delete(`http://localhost:8800/service/${id}`, {
        headers: {
          authorization: bearer,
        },
      })
