...
const history=useNavigate();
  ...
  const getQuantite = async (id) => {
    axios
      .get(`http://localhost:8800/user/autorisation/${id}`, {
        headers: {
  ...
  const getMaterielle = async (id) => {
    axios
      .get(`http://localhost:8800/biens_societe/materielles/${id}`, {
        headers: {
          authorization: bearer,
        },
      })
  ...
  const getCount=async(id)=>{
       axios.get(`http://localhost:8800/materielleCount/${id}`,{
        headers: {
          authorization: bearer,
        },
      }).then((data)=>{
  ...
  const addData = (values) => {
    ...
    axios
      .post(`http://localhost:8800/materielles/${x}`, information, {
        headers: {
          authorization: bearer,
        },
      })
