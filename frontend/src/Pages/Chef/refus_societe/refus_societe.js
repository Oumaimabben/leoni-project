...
const Refus_societe = () => {
  ...
  const handleConfirm = () => {
    if (seletectedItem != null) {
      axios
        .patch(
          "http://localhost:8800/admin/biens_societe/" + seletectedItem,
          { etat: "Refuse" },
          {
            headers: {
        ...
        .then((data) => {
          notification.success({ message: "autorisation refusée" });
          toggleConfirmModal();
          getData();
        })
        .catch((err) => {
  ...
  const getData = () => {
    axios
      .get("http://localhost:8800/admin/biens_societe/?status=En_attente", {
        headers: {
          authorization: bearer,
        },
      })
