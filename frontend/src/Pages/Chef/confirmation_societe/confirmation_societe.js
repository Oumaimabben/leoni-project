...
const Confirmation_societe = () => {
    ...
    const handleConfirm = () => {
      if (seletectedItem != null) {
         axios
          .patch(
            "http://localhost:8800/admin/biens_societe/" + seletectedItem,{etat:"Accepte"},
            {
                headers: {
    ...
    const columns = [
      ...
      },
      {
        ...
        render:(action)=>(
          <>
            <Button type="primary" htmlType='button' style={{ background: "green",border:"0px" }}  onClick={()=>{
    ...
    const getData = () => {
      axios
        .get(
          "http://localhost:8800/admin/biens_societe/?status=En_attente",
          {
            headers: {
              authorization: bearer,
            },
...
export default Confirmation_societe