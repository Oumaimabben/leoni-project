...
const Confirmation_personel = () => {
    ...
    const handleConfirm = () => {
      if (seletectedItem != null) {
         axios
          .patch(
            "http://localhost:8800/admin/biens_personel/" + seletectedItem,{etat:"Accepte"},
            {
                headers: {
    ...
    const getData = () => {
      axios
        .get(
          "http://localhost:8800/admin/biens_personel/?status=En_attente",
          {
            headers: {
              authorization: bearer,
            },
    ...
    return (
      <UserDashboard>
        <div>
          {confirmModal && (
            <Modal
              title={`accept l'autorisation n° ${seletectedItem}`}
...
export default Confirmation_personel